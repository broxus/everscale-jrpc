...
fn serialize_contract_state(bench: &mut Criterion) {
        ...
        result: Some(rpc::response::Result::GetContractState(
            rpc::response::GetContractState {
                contract_state: Some(rpc::response::get_contract_state::ContractState {
                    account: Bytes::from(account.write_to_bytes().unwrap()),
                    gen_timings: Some(rpc::response::get_contract_state::contract_state::GenTimings::Unknown(Default::default())),
                    last_transaction_id: Some(rpc::response::get_contract_state::contract_state::LastTransactionId::Inexact(
                        Default::default(),
                    )),
                }),
            },
    ...
    bench.bench_function("serialize proto", |b| {
        b.iter(|| {
            let _bytes = std::hint::black_box(&state_response).encode_to_vec();
        })
    });
}
...
fn deserialize_contract_state(bench: &mut Criterion) {
        ...
        result: Some(rpc::response::Result::GetContractState(
            rpc::response::GetContractState {
                contract_state: Some(rpc::response::get_contract_state::ContractState {
                    account: Bytes::from(account.write_to_bytes().unwrap()),
                    gen_timings: Some(
                        rpc::response::get_contract_state::contract_state::GenTimings::Unknown(
                            Default::default(),
                        ),
                    ),
                    last_transaction_id: Some(rpc::response::get_contract_state::contract_state::LastTransactionId::Inexact(
    ...
    bench.bench_function("deserialize proto", |b| {
        b.iter(|| {
            let _decoded = rpc::Response::decode(bytes.as_slice())
