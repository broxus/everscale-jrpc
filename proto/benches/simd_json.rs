...
fn serialize_contract_state(bench: &mut Criterion) {
    ...
    let contract_state = GetContractStateResponse::Exists {
        account: account.write_to_bytes().unwrap(),
        timings: nekoton_abi::GenTimings::Known {
            gen_lt: Default::default(),
            gen_utime: Default::default(),
        },
        last_transaction_id: nekoton_abi::LastTransactionId::Inexact {
            latest_lt: Default::default(),
        },
    };

    bench.bench_function("serialize simd json", |b| {
        b.iter(|| {
            let _value = simd_json::to_string(std::hint::black_box(&contract_state))
...
fn deserialize_contract_state(bench: &mut Criterion) {
    ...
    let mut value = serde_json::to_string(&contract_state).expect("Failed to serialize");

    bench.bench_function("deserialize simd json", |b| {
        b.iter(|| unsafe {
            let _decoded: GetContractStateResponse =
                simd_json::from_str(&mut value).expect("Failed to deserialize");
        })
    });
}
