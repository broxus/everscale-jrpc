...
fn serialize_contract_state(bench: &mut Criterion) {
    ...
    bench.bench_function("serialize json", |b| {
        b.iter(|| {
            let _value = serde_json::to_string(std::hint::black_box(&contract_state))
...
fn deserialize_contract_state(bench: &mut Criterion) {
    ...
    let value = serde_json::to_string(&contract_state).expect("Failed to serialize");

    bench.bench_function("deserialize json", |b| {
        b.iter(|| {
            let _decoded: GetContractStateResponse =
                serde_json::from_str(&value).expect("Failed to deserialize");
        })
    });
}
